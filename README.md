...
In this project, you will implement three algorithms for policy evaluation in Blackjack: Monte Carlo (MC) Policy Evaluation, Temporal Difference (TD) Policy Evaluation, and Q-Learning. The base game engine, derived from [this repository](https://github.com/ServePeak/Blackjack-Python/blob/master/blackjack.py), provides the framework for your AI implementation.

**Game Overview:**
Blackjack is a popular card game where the objective is to achieve a sum closer to 21 than the dealer without exceeding it. The game engine simulates Blackjack with minor simplifications. Players can hit or stand to adjust their card sums, aiming to beat the dealer.

**Tasks:**
1. **Monte Carlo Policy Evaluation**: Implement this method to evaluate a simple policy where the player hits if their card sum is below 14 and stands otherwise.
2. **Temporal Difference Policy Evaluation**: Similarly, evaluate the same policy using the TD method.
3. **Q-Learning**: Develop the Q-learning algorithm with an epsilon-greedy strategy (epsilon = 0.4) to learn optimal actions. The learned policy will be tested using an autoplay feature.
...
python main.py
```
Use buttons to control the game manually or toggle learning algorithms. Save or load AI states with keyboard shortcuts for testing.

**Testing:**
- `python main.py -t 1 -a 1` for deterministic tests.
- `python main.py -t 2` for convergence tests.

Implementing these algorithms will help you understand key concepts in reinforcement learning and policy evaluation. Make sure to read the provided game engine code and the detailed task instructions to successfully complete the project.
...
1. Cards are drawn with replacement, so

 assume that you are playing with infinitely many decks of cards and the drawn cards do not affect the probability of the next cards. 
